...
import type { CellData, CellDataMap, CellPosition, CellStyle, ConditionalFormatRule, FilterCondition, FilterRange, MergeInfo, NamedRange, PivotTableConfig, SelectionRange, SheetData, SortKey, ValidationRule, WorkbookData } from '../types/grid';
...
import type { FormulaResult, FunctionReturnValue, NamedRangeResolver, RangeExpander, SheetNameResolver } from '../engine/types';
...
import { createEmptySheet, generateSheetName, generateCopyName, deepCloneSheet, generateSheetId } from '../utils/sheetUtils';
import { resolveSpill, clearSpillRange } from '../engine/spill';
import type { SparklineConfig } from '../types/sparkline';
import { addGroup as addGroupFn, removeGroup as removeGroupFn, toggleGroupCollapse as toggleGroupCollapseFn, setExpandLevel as setExpandLevelFn, shiftGroupRanges } from '../grouping/groupManager';
...
function toParseErrorInfo(e: unknown): CellData['parseError'] {
  return e instanceof FormulaSyntaxError ? { message: e.message, start: e.start, end: e.end } : undefined;
}
...
export interface UseGridDataReturn {
  ...
  batchSetCellValuesOnSheet: (sheetId: string, entries: Array<{ col: number; row: number; value: string }>) => void;
  ...
  insertColumn: (colIndex: number, position: 'before' | 'after', colCount: number, maxCols: number) => boolean;
  /** Delete a column */
  deleteColumn: (colIndex: number) => boolean;
  /** Insert a row before or after the given index */
  insertRow: (rowIndex: number, position: 'before' | 'after', rowCount: number, maxRows: number) => boolean;
  /** Delete a row */
  ...
  sortRange: (range: { startCol: number; endCol: number; startRow: number; endRow: number }, keys: SortKey[], onSorted?: (order: number[]) => void) => boolean;
  ...
  trimWhitespaceInRange: (range: { startCol: number; endCol: number; startRow: number; endRow: number }) => void;
...
function hydratePlainCells(sheets: SheetData[]): void {
  for (const sheet of sheets) {
    for (const cell of sheet.cells.values()) {
      ...
      if (cell.formula === undefined && !cell.spillSource && cell.rawValue.length > 1 && cell.rawValue.startsWith('=')) {
        cell.formula = cell.rawValue.slice(1);
        cell.computed = undefined;
        continue;
      }
      if (cell.formula !== undefined || cell.spillSource || cell.computed !== undefined || cell.rawValue === '') continue;
      const parsed = cell.style?.numberFormat === 'plainText' ? { value: cell.rawValue } : parseUserInput(cell.rawValue);
...
function shiftSparklineRefsForCol(sparklines: SparklineConfig[], colIndex: number, op: 'insert' | 'delete'): SparklineConfig[] {
...
function shiftSparklineRefsForRow(sparklines: SparklineConfig[], rowIndex: number, op: 'insert' | 'delete'): SparklineConfig[] {
  return sparklines.map(s => {
...
export function useGridData(): UseGridDataReturn {
  ...
  const {
    pushSnapshot, undo: undoStack, redo: redoStack, canUndo, canRedo, clearHistory,
    timeline: historyTimeline, getTimelineSnapshot, jumpTo: jumpHistoryTo,
  } = useUndoRedo();
  ...
  const resolveSheetName: SheetNameResolver = useCallback((sheetName: string): string | undefined => {
  ...
  const namedRangeResolverFor = useCallback((sheetId: string): NamedRangeResolver => {
    return (name: string) => {
      ...
      if (rangeStr.includes(':')) {
        ...
        return { keys: addPrefix ? keys.map(k => `${targetSheetId}:${k}`) : keys, sheetId: targetSheetId };
      }
      return { keys: addPrefix ? [`${targetSheetId}:${rangeStr}`] : [rangeStr], sheetId: targetSheetId };
    };
  }, []);
  ...
  function rangeLabel(r: { startCol: number; startRow: number; endCol: number | null; endRow: number | null }): string {
    if (r.endRow === null) {
      // Column-wide / end-open range: 'A:C' if the whole column, else 'A2:C'
      const startLabel = r.startRow === 0 ? colIndexToLetter(r.startCol) : `${colIndexToLetter(r.startCol)}${r.startRow + 1}`;
   ...
   */
  const resolveCell = useCallback((key: string): FormulaResult => {
   ...
   */
  const evaluateCell = useCallback((gKey: string, passCache?: PassCache): string[] => {
    ...
    try {
      ...
      const result = evaluate(ast, sheetResolver, expandRange, resolveSheetName, namedRangeResolverFor(sheetId), setHyperlinkMeta, {
      ...
      if (isSpillResult(result)) {
        ...
        if ('error' in spillRes) {
          ...
          cell.error = originIsError ? (originVal as import('../engine/types').FormulaError).code : undefined;
    ...
    return touched;
  }, [resolveCell, parseGlobalKey, getSheetById, globalKey, resolveSheetName, namedRangeResolverFor, getUsedBounds]);
   ...
   */
  const recalculateDependents = useCallback((changedGlobalKeys: string[], alsoEvaluate: string[] = []): void => {
    ...
    for (let iter = 0; iter < 10 && (pending.length > 0 || extra.length > 0); iter++) {
      ...
      const passCache: PassCache = { rangeCache: new Map(), callCache: new Map(), passStats: rec?.stats };
      const touched: string[] = [];
      if (rec) {
        rec.iterations++;
  ...
  const getCellData = useCallback((col: number, row: number): CellData | undefined => {
   ...
   */
  const setCellValueInternal = useCallback((col: number, row: number, value: string, targetSheet?: SheetData, deferEval?: string[]): string => {
    ...
    if (value.startsWith('=') && value.length > 1) {
      ...
      try {
        ...
        const globalDeps = refs.cells.map(dep => (dep.includes(':') ? dep : globalKey(sheet.id, dep)));
        const rangeDeps: GlobalRangeDep[] = refs.ranges.map(r => ({
          sheetId: r.sheetId ?? sheet.id,
          startCol: r.startCol,
          startRow: r.startRow,
        ...
        const depLabels = [...globalDeps, ...rangeDeps.map(r => `${r.sheetId}:${rangeLabel(r)}`)];
        ...
        const result = evaluate(ast, sheetResolver, expandRange, resolveSheetName, resolveNR, setHyperlinkMeta, {
        ...
        if (isSpillResult(result)) {
          ...
          if ('error' in spillRes) {
            ...
            sheet.cells.set(key, {
              ...
              formula: formulaStr,
              dependencies: depLabels,
              error: originIsError ? (originVal as import('../engine/types').FormulaError).code : undefined,
      ...
      const parsed = existingStyle?.numberFormat === 'plainText' ? { value } : parseUserInput(value);
      // Auto-apply the detected format (e.g. "10%" → percent) only while the cell's format is automatic
      const hasExplicitFormat = existingStyle?.numberFormat !== undefined && existingStyle.numberFormat !== 'auto';
      const style = parsed.formatHint && !hasExplicitFormat
        ? { ...existingStyle, numberFormat: parsed.formatHint.numberFormat, numberFormatPattern: parsed.formatHint.pattern }
        : existingStyle;
    ...
    return gKey;
  }, [resolveCell, getActiveSheet, globalKey, resolveSheetName, namedRangeResolverFor, getUsedBounds]);
  ...
  const deleteCells = useCallback((positions: CellPosition[]) => {
    ...
    bumpVersion();
  }, [getActiveSheet, pushWorkbookSnapshot, setCellValueInternal, recalculateDependents, bumpVersion]);
  ...
  const batchSetCellValues = useCallback(
    (entries: Array<{ col: number; row: number; value: string }>) => {
      ...
      for (const entry of entries) {
        const gKey = setCellValueInternal(entry.col, entry.row, entry.value, undefined, deferredFormulas);
   ...
   */
  const restoreFromSnapshot = useCallback((snapshot: WorkbookSnapshot) => {
  ...
  const getHistorySnapshot = useCallback((index: number): WorkbookSnapshot | undefined => {
  ...
  const getDependencyInfo = useCallback((sheetId: string, key: string) => {
    const graph = depGraphRef.current;
    ...
    (index: number, operation: 'insert' | 'delete') => {
      ...
      for (const other of sheetsRef.current) {
        ...
        for (const [otherKey, otherCell] of other.cells) {
          ...
          const res = updateReferences(otherCell.formula, 'column', index, operation, { appliesToUnqualified: false, targetSheetName: sheet.name });
          if (res.formula === otherCell.formula) continue;
          other.cells.set(otherKey, {
            ...otherCell,
            rawValue: '=' + res.formula,
            formula: res.formula,
            ...(res.hasRefError ? { displayValue: '#REF!', error: '#REF!', computed: undefined } : {}),
          });
        }
      }
      ...
      for (const [key, cell] of oldMap) {
        ...
        if (cell.rawValue.startsWith('=') && cell.formula) {
          const { formula: updatedFormula, hasRefError } = updateReferences(cell.formula, 'column', index, operation, { targetSheetName: sheet.name });
  ...
  const shiftCellDataForRow = useCallback(
    (index: number, operation: 'insert' | 'delete') => {
      ...
      for (const other of sheetsRef.current) {
        ...
        for (const [otherKey, otherCell] of other.cells) {
          if (!otherCell.formula) continue;
          const res = updateReferences(otherCell.formula, 'row', index, operation, { appliesToUnqualified: false, targetSheetName: sheet.name });
          if (res.formula === otherCell.formula) continue;
          other.cells.set(otherKey, {
            ...otherCell,
            rawValue: '=' + res.formula,
      ...
      for (const [key, cell] of oldMap) {
        ...
        // Update formula references
        if (cell.rawValue.startsWith('=') && cell.formula) {
          const { formula: updatedFormula, hasRefError } = updateReferences(cell.formula, 'row', index, operation, { targetSheetName: sheet.name });
          const newRawValue = '=' + updatedFormula;
   ...
   */
  function shiftHiddenIndices(indices: number[] | undefined, index: number, op: 'insert' | 'delete'): number[] | undefined {
  ...
  const insertColumn = useCallback(
    (colIndex: number, position: 'before' | 'after', colCount: number, maxCols: number): boolean => {
    ...
    [pushWorkbookSnapshot, shiftCellDataForColumn, getActiveSheet, rebuildGlobalDepGraph, recalculateAll, bumpVersion],
  );
  ...
  const insertRow = useCallback(
    (rowIndex: number, position: 'before' | 'after', rowCount: number, maxRows: number): boolean => {
    ...
    [pushWorkbookSnapshot, shiftCellDataForRow, getActiveSheet, rebuildGlobalDepGraph, recalculateAll, bumpVersion],
  );
  ...
  const hideRows = useCallback((start: number, end: number): void => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    const lo = Math.min(start, end);
    const hi = Math.max(start, end);
  ...
  const unhideRows = useCallback((start: number, end: number): void => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    const lo = Math.min(start, end);
    const hi = Math.max(start, end);
  ...
  const hideCols = useCallback((start: number, end: number): void => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    const lo = Math.min(start, end);
    const hi = Math.max(start, end);
  ...
  const unhideCols = useCallback((start: number, end: number): void => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    const lo = Math.min(start, end);
    const hi = Math.max(start, end);
    ...
    for (const sheet of sheetsRef.current) {
      for (const [key, cell] of sheet.cells) {
        if (cell.formula && cell.dependencies) {
          const hasDeletedRef = cell.dependencies.some(dep => dep.startsWith(deletedSheet.id + ':'));
          if (hasDeletedRef) {
            sheet.cells.set(key, {
              ...cell,
              displayValue: '#REF!',
  ...
  const duplicateSheet = useCallback((sheetId: string): string | null => {
    const idx = sheetsRef.current.findIndex(s => s.id === sheetId);
    ...
    const newSheet: SheetData = {
      ...
      id: newId,
      name: newName,
      charts: cloned.charts.map(c => ({ ...c, id: Date.now().toString(36) + Math.random().toString(36).slice(2, 8) })),
      sparklines: cloned.sparklines.map(s => ({ ...s, id: Date.now().toString(36) + Math.random().toString(36).slice(2, 8) })),
      conditionalFormatRules: cloned.conditionalFormatRules.map(r => ({ ...r, id: `cfr_${Date.now()}_${Math.random().toString(36).slice(2, 8)}` })),
    };
  ...
  const moveSheet = useCallback((sheetId: string, toIndex: number): void => {
    const idx = sheetsRef.current.findIndex(s => s.id === sheetId);
    if (idx === -1) return;
  ...
  const setSheetTabColor = useCallback((sheetId: string, color: string | undefined): void => {
    const sheet = getSheetById(sheetId);
    if (!sheet) return;
  ...
  const setSheetHidden = useCallback((sheetId: string, hidden: boolean): boolean => {
    const sheet = getSheetById(sheetId);
    ...
    if (hidden && activeSheetIdRef.current === sheetId) {
      ...
      for (let i = idx + 1; i < sheetsRef.current.length; i++) {
        if (!sheetsRef.current[i].hidden) { next = sheetsRef.current[i]; break; }
      }
      if (!next) {
  ...
  const setColCount = useCallback((count: number) => {
    const sheet = getActiveSheet();
  ...
  const setFilterState = useCallback((state: Map<number, Set<string>>) => {
  ...
  const createFilter = useCallback((range: FilterRange) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
  ...
  const setFilterCondition = useCallback((col: number, condition: FilterCondition | undefined) => {
    const sheet = getActiveSheet();
  ...
  const sortRange = useCallback((
    ...
    const getValue = (col: number, row: number): FormulaResult => {
      ...
      if (!cell) return '';
      if (cell.error) return { type: 'error' as const, code: cell.error as import('../engine/types').ErrorCode };
      if (cell.computed !== undefined) return cell.computed;
      return '';
    };
   ...
   */
  const splitTextToColumns = useCallback((col: number, startRow: number, endRow: number, delimiter: string) => {
    const sheet = getActiveSheet();
    const entries: Array<{ col: number; row: number; value: string }> = [];
   ...
   */
  const replaceCellDataDirect = useCallback((newData: CellDataMap) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    sheet.cells = newData;
    hydratePlainCells([sheet]);
   ...
   */
  const clearCellsOnSheet = useCallback((sheetId: string, positions: Array<{ col: number; row: number }>) => {
    const sheet = getSheetById(sheetId);
    if (!sheet) return;
    const graph = depGraphRef.current;
    for (const pos of positions) {
  ...
  const addConditionalFormatRule = useCallback((rule: Omit<ConditionalFormatRule, 'id'>) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    const id = `cfr_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const newRule: ConditionalFormatRule = { ...rule, id, range: { ...rule.range }, style: { ...rule.style } };
    sheet.conditionalFormatRules = [...(sheet.conditionalFormatRules ?? []), newRule];
    bumpVersion();
  }, [pushWorkbookSnapshot, getActiveSheet, bumpVersion]);

  const updateConditionalFormatRule = useCallback((ruleId: string, updates: Partial<ConditionalFormatRule>) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    sheet.conditionalFormatRules = (sheet.conditionalFormatRules ?? []).map(r =>
      r.id === ruleId ? { ...r, ...updates, range: updates.range ? { ...updates.range } : { ...r.range }, style: updates.style ? { ...updates.style } : { ...r.style } } : r,
    );
    bumpVersion();
  }, [pushWorkbookSnapshot, getActiveSheet, bumpVersion]);

  const deleteConditionalFormatRule = useCallback((ruleId: string) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
  ...
  const replaceWorkbook = useCallback((workbook: WorkbookData, options?: { resetHistory?: boolean }) => {
  ...
  // --- Cell merge operations ---
  const mergeCells = useCallback((range: SelectionRange) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
  ...
  const unmergeCells = useCallback((anchorKey: string) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
  ...
  const getMergeInfo = useCallback((col: number, row: number): MergeInfo | undefined => {
    const sheet = getActiveSheet();
    return sheet.merges?.get(cellKey(col, row));
  }, [getActiveSheet]);

  // --- Cell comment operations ---
  const setCellComment = useCallback((col: number, row: number, comment: string | undefined) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    const key = cellKey(col, row);
    const existing = sheet.cells.get(key);
  ...
  // --- Data validation operations ---
  const setValidationRule = useCallback((positions: CellPosition[], rule: ValidationRule | undefined) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    for (const pos of positions) {
      const key = cellKey(pos.col, pos.row);
  ...
  // --- Sparkline operations ---
  const addSparkline = useCallback((config: import('../types/sparkline').SparklineConfig) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
  ...
  const updateSparkline = useCallback((id: string, updates: Partial<import('../types/sparkline').SparklineConfig>) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
  ...
  const deleteSparkline = useCallback((id: string) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    if (!sheet.sparklines) return;
  ...
  // --- Grouping operations ---
  const addRowGroup = useCallback((start: number, end: number): boolean => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
  ...
  const addColGroup = useCallback((start: number, end: number): boolean => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
  ...
  const removeRowGroup = useCallback((groupId: string) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
  ...
  const removeColGroup = useCallback((groupId: string) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
  ...
  const toggleRowGroupCollapse = useCallback((groupId: string) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    if (!sheet.rowGroups) return;
  ...
  const toggleColGroupCollapse = useCallback((groupId: string) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    if (!sheet.colGroups) return;
  ...
  const setRowExpandLevel = useCallback((level: number) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    if (!sheet.rowGroups) return;
  ...
  const setColExpandLevel = useCallback((level: number) => {
    pushWorkbookSnapshot();
    const sheet = getActiveSheet();
    if (!sheet.colGroups) return;
  ...
  const addNamedRange = useCallback((name: string, range: string, refSheetId?: string): boolean => {
    ...
    bumpVersion();
    return true;
  }, [isValidNamedRangeName, pushWorkbookSnapshot, rebuildGlobalDepGraph, recalculateAll, bumpVersion]);

  const updateNamedRange = useCallback((oldName: string, newName: string, range: string, refSheetId?: string): boolean => {
      ...
      for (const sheet of sheetsRef.current) {
        for (const [key, cell] of sheet.cells) {
          if (cell.formula && cell.rawValue.startsWith('=')) {
            ...
            const regex = new RegExp(`(?<![\\p{L}\\p{N}_])${escapeRegExp(oldName)}(?![\\p{L}\\p{N}_])`, 'gu');
  ...
  const deleteNamedRange = useCallback((name: string): boolean => {
  ...
  // --- Pivot table operations ---
  const addPivotTable = useCallback((config: PivotTableConfig) => {
    pushWorkbookSnapshot();
    pivotTablesRef.current = [...pivotTablesRef.current, { ...config }];
    bumpVersion();
  }, [pushWorkbookSnapshot, bumpVersion]);

  const updatePivotTable = useCallback((id: string, updates: Partial<PivotTableConfig>) => {
    pushWorkbookSnapshot();
  ...
  const deletePivotTable = useCallback((id: string) => {
    pushWorkbookSnapshot();
  ...
  // --- Formula evaluation helper (for function wizard) ---
  const evaluateFormulaHelper = useCallback((formula: string): string => {
    try {
      ...
      const result = evaluate(ast, sheetResolver, expandRange, resolveSheetName, namedRangeResolverFor(sheetId), undefined, {
        getSheetBounds: sid => getUsedBounds(sid ?? sheetId),
      });
      if (isSpillResult(result)) {
  ...
  const evaluateFormulaAt = useCallback((formula: string, col: number, row: number, sheetId?: string): FormulaResult => {
    ...
    try {
      ...
      const result = evaluate(ast, sheetResolver, expandRange, resolveSheetName, namedRangeResolverFor(targetSheetId), undefined, {
  ...
  const resolveRangeValues = useCallback((range: string, sheetId?: string): FormulaResult[] | null => {
...
function updateSheetNameInFormula(formula: string, oldName: string, newName: string): string {
  ...
  let updated = formula.replace(quotedPattern, needsQuoting(newName) ? `'${newName}'!` : `${newName}!`);
  ...
  updated = updated.replace(unquotedPattern, needsQuoting(newName) ? `'${newName}'!` : `${newName}!`);
